...
type Client struct {
	...
	sendingMethodHandler   MethodHandler[*ClientSession]
	receivingMethodHandler MethodHandler[*ClientSession]
}
...
func NewClient(name, version string, opts *ClientOptions) *Client {
	c := &Client{
		name:                   name,
		version:                version,
		sendingMethodHandler:   defaultSendingMethodHandler[*ClientSession],
		receivingMethodHandler: defaultReceivingMethodHandler[*ClientSession],
	}
	if opts != nil {
		c.opts = *opts
	}
...
func (c *Client) AddSendingMiddleware(middleware ...Middleware[*ClientSession]) {
	c.mu.Lock()
	defer c.mu.Unlock()
	addMiddleware(&c.sendingMethodHandler, middleware)
}
...
func (c *Client) AddReceivingMiddleware(middleware ...Middleware[*ClientSession]) {
	c.mu.Lock()
	defer c.mu.Unlock()
	addMiddleware(&c.receivingMethodHandler, middleware)
}
...
func (cs *ClientSession) sendingMethodHandler() methodHandler {
	...
	return cs.client.sendingMethodHandler
}

func (cs *ClientSession) receivingMethodHandler() methodHandler {
	cs.client.mu.Lock()
	defer cs.client.mu.Unlock()
	return cs.client.receivingMethodHandler
}
